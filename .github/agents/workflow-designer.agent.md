...
2. IRスキーマ (`docs/ir/{{name}}-ir.schema.md`) ※必要に応じて
...
- 3回確認しても要件が不明確な場合は、最小構成で仮実装を提案する
...
User: Azureの環境情報を取得してレポート化したい
→ ヒアリング → IR生成 → エージェント定義生成
結果: azure-reporter.agent.md + レポートテンプレート

### 例2: コードレビュー自動化

User: PRのコードを自動レビューしたい
→ ヒアリング → IR生成 → オーケストレーター + ワーカー生成
結果: review-orchestrator.agent.md + analyzer.agent.md + reviewer.agent.md

### 例3: ドキュメント生成

User: ソースコードからAPI仕様書を自動生成したい
→ ヒアリング → IR生成 → エージェント定義生成
